...
bool Gxt2File::save(const std::wstring& filePath) const
{
    ...
    for (const auto& p : m_entries) {
        ...
        currentOffset += static_cast<uint32_t>(utf8str.size() + 1); // NULL終端含む
        idx++;
    }
    ...
    for (const auto& s : utf8Strings) {
        file.write(s.c_str(), s.size() + 1); // NULL終端込み
    }
